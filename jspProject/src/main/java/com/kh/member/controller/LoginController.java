...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//	  해당 요청을 처리하는 서비스 클래스의 메서드 호출
		
		Member loginUser = new MemberService().loginMember(userId, userPwd);
		
		/**
		 * 
		 * 응답페이지 전달할 값이 있을 경우 어딘가에 담아야한다. (담을 수 있는 영역 == JSP 내장객체 4종류)
		 ...
		 * 			데이터를 꺼내고자한다면 특정객체.getAttribute("키") : Object타입 반환
		...
		if (loginUser == null) {
			...
			// 해당 방식으로 하면 응답시 url은 http://localhost:8002/jsp/login.me가 나타나며 실제 화면은 localhost:8002/jsp/의 응답화면이 나타나게 된다.
			// 응답페이지(jsp)에게 위임시 필요한 객체(RequestDispatcher)
