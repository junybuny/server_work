...
public class BoardInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (ServletFileUpload.isMultipartContent(request)) {
			// 파일업로드를 위한 라이브러리 : cos.jar(com.oreilly.servlet의 약자)
			// www.servlets.com cos다운로드
			 ...
			 * 단, 업로드시 파일명을 수정해주는게 일반적이다. 그래서 파일명을 수정시켜주는 객체를 제시해야한다.
			 * => 같은 파일명이 존재할경우 덮어씌워질 수 있고, 파일명에 한글/특수문자/띄어쓰기가 포함될 경우 서버에 따라 문제발생
			 * 
			 * 기본적으로 파일명이 안겹치도록 수정작업해주는 객체가 있다.
			 ...
			 * 		return 수정파일
			 * }
			 * 위와같은 방식이기때문에 사용하지 않는다.
			 * 
			 * 나만의 방식대로 절대 안겹치도록 rename 할수있게 FileRenamePolicy클래스 만들기
